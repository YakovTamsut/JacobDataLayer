...
namespace ViewModel
{
    public class WorkoutPlanDB : BaseDB
    {
        ...
        public WorkoutPlan GetWorkoutPlanByTommorowAndUser(User user)
        {
            int[] days = new int[] { 1, 2, 3, 4, 5, 6, 7 };
            command.CommandText = $"SELECT * FROM tblUserWorkPlan WHERE userID = {user.ID} AND day = {days[(int)DateTime.Today.DayOfWeek]+1}";
            WorkoutPlanList wp = new WorkoutPlanList(ExecuteCommand());
            if (wp.Count == 0) return null;
            return wp[0];
        }
        ...
        {
            command.CommandText = $"DELETE FROM tblUserWorkPlan WHERE userID = {user.ID} AND WorkoutID = {workout.ID}";
            return ExecuteCRUD();
        }
